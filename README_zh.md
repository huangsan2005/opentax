...
- [ ] **中国税种目录覆盖：18 个法定税种已入库 8 个**（增值税、企税、个税、土增税、烟叶税、城建税完整；消费税、印花税部分）。贡献者优先补缺顺序：车辆购置税（10% 比例，最适合作为第一个 PR 练手）、契税（3–5% 省级差异，地区样本）、房产税（从价 1.2%/从租 12%）、印花税全目录（13 个税目）、车船税（定额税目表）；关税/资源税/环保税复杂度高放最后。

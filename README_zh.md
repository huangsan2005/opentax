...
| **日本消费税**（同一 id 四个历史版本 3%→5%→8%→10% 挂时间轴，交易日自动选版；軽減税率8%；堂食/外带对比场景） | `flat` 四版本时间轴 | 消費税法（昭和63年法律第108号）及历次改正 |
...
- [x] **新增三税种（2026-08-28，来源=税务总局政策法规库一手全文）**：车辆购置税 10%（法律级）、契税 3-5% 幅度下限、房产税从价 1.2%/从租 12% —— 中国税种目录覆盖升至 **11/18**
- [x] **产权转移书据入库**（印花税法附表：土地出让/房屋/股权转让 万分之五，商标/著作权/专利 万分之三——山西省税务局官方问答页逐字引用附表核验）
- [x] **车船税乘用车7档入库**（法定区间全国统一、税额省定——北京标准从北京市税务局官方附件doc解析核验；其他省按同id多版本补充）
- [ ] **中国税种目录覆盖：约 12.5/18**（消费税部分、印花税 3/13 税目）。剩余缺口优先级：印花税其余 11 目、城镇土地使用税、契税省级版本与个人购房优惠、消费税其余税目；关税/资源税/环保税最后。
...
- [x] 日本消费税样本——EU 的镜像：同一 id 四个历史税率版本挂时间轴（3%→5%→8%→10%），同一条管线只改日期就能在四个税制时代穿梭；堂食10%对比外带8%（"イートイン戦争"）
